...
class MusicListComponent extends CBitrixComponent
{
	...
	public function onPrepareComponentParams($arParams): array
	{
		...
		if (isset($arParams['GENRE'])&&!preg_match('/^[A-Za-z]+$/',$arParams['GENRE']))
		{
			throw new ObjectPropertyException('Invalid genre');
		}

		if (!preg_match('/^[\/A-Za-z\d]+$/',$arParams['PAGINATION_BASE_LINK']))
		{
			throw new ObjectPropertyException('Invalid pagination link');
		}
	...
	protected function prepareTemplateParams(): void
	{
		...
		if(isset($this->arParams['GENRE']))
		{
			$this->arResult['GENRE'] = $this->arParams['GENRE'];
		}

		if(isset($this->arParams['USER_NICKNAME']))
		{
			$this->arResult['USER_NICKNAME'] = $this->arParams['USER_NICKNAME'];
		}
	}
}