    ...
    <div class="hero-body is-justify-content-center is-align-items-center">
        <div class="columns is-flex is-flex-direction-column box">
            ...
            <? else: ?>
                    ...
                    <? foreach ($arResult["SHOW_FIELDS"] as $FIELD): ?>
                        <? if ($FIELD == "AUTO_TIME_ZONE" && $arResult["TIME_ZONE_ENABLED"] == true): ?>
                            ...
                            <select name="REGISTER[AUTO_TIME_ZONE]"
                                ...
                                <option
                                    value=""><? echo GetMessage("main_profile_time_zones_auto_def") ?></option>
                                <option
                            ...
                            <select
                                name="REGISTER[TIME_ZONE]"<? if (!isset($_REQUEST["REGISTER"]["TIME_ZONE"])) echo 'disabled="disabled"' ?>>
                                <? foreach ($arResult["TIME_ZONE_LIST"] as $tz => $tz_name): ?>
                                    <option
                                        value="<?= htmlspecialcharsbx($tz) ?>"<?= $arResult["VALUES"]["TIME_ZONE"] == $tz ? " selected=\"selected\"" : "" ?>><?= htmlspecialcharsbx($tz_name) ?></option>
                                <? endforeach ?>
                            </select>
				...
				<span class="bx-auth-secure" title="<? echo GetMessage("AUTH_NONSECURE_NOTE") ?>">
					<div class="bx-auth-secure-icon bx-auth-secure-unlock"></div>
                                ...
                                case "CONFIRM_PASSWORD":
                                    ?><input class="input is-primary" size="30" type="password"
                                             name="REGISTER[<?= $FIELD ?>]"
                                ...
                                case "WORK_COUNTRY":
                                    ...
                                    foreach ($arResult["COUNTRIES"]["reference_id"] as $key => $value) {
                                        ?>
                                        <option
                                            value="<?= $value ?>"<? if ($value == $arResult["VALUES"][$FIELD]): ?> selected="selected"<? endif ?>><?= $arResult["COUNTRIES"]["reference"][$key] ?></option>
                                        <?
                                    }
                                    ?></select><?
                                    break;
                                ...
                                case "PERSONAL_PHOTO":
                                case "WORK_LOGO":
                                    ?><input class="input is-primary" size="30" type="file"
                            ...
                            if ($FIELD == "PERSONAL_BIRTHDAY"): ?>
                                ...
                                ?><input class="input is-primary" size="30" type="text"
                                         name="REGISTER[<?= $FIELD ?>]"
                                         value="<?= $arResult["VALUES"][$FIELD] ?>" /><?
                                if ($FIELD == "PERSONAL_BIRTHDAY")
                    ...
                    if ($arResult["USE_CAPTCHA"] == "Y") {
                        ...
                        <b><?= GetMessage("REGISTER_CAPTCHA_TITLE") ?></>
                    ...
                    <div class="field">
                        <input class="button is-success" type="submit" name="register_submit_button"
                ...
                <p class="password_requirements"><? echo $arResult["GROUP_POLICY"]["PASSWORD_REQUIREMENTS"]; ?></p>
            ...
            <noindex>
                <div class="bx-authform-link-container">
                    <a href="/login/"
                       rel="nofollow"><b>Уже есть аккаунт? Авторизируйтесь.</b></a>
                </div>
            </noindex>
        </div>
    </div>
